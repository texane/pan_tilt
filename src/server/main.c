...
static int begin_request_handler(struct mg_connection *conn) {
  ...
  if (!strcmp(ri->uri, "/handle_post_request")) {
    ...
    if (strcmp(input1, "pan_cw") == 0) pt_pan_cw(&pt_dev);
    else if (strcmp(input1, "pan_ccw") == 0) pt_pan_ccw(&pt_dev);
    else if (strcmp(input1, "tilt_cw") == 0) pt_tilt_cw(&pt_dev);
    else if (strcmp(input1, "tilt_ccw") == 0) pt_tilt_ccw(&pt_dev);
