...
static pt_dev_t pt_dev;

static void init_pt_once(void)
{
...
static void do_pan_cw(void)
{
  init_pt_once();
  pt_pan_cw(&pt_dev);
}

static void do_pan_ccw(void)
{
  init_pt_once();
  pt_pan_ccw(&pt_dev);
}

static void do_tilt_cw(void)
{
  init_pt_once();
  pt_tilt_cw(&pt_dev);
}

static void do_tilt_ccw(void)
{
  init_pt_once();
  pt_tilt_ccw(&pt_dev);
}
...
void *client_thread(void *arg)
{
    ...
    if(strstr(buffer, "GET /?action=snapshot") != NULL) {
        ...
        input_suffixed = 255;
    } else if(strstr(buffer, "GET /stream.html?/tilt_ccw") != NULL) {
        req.type = A_TILT_CCW;
        input_suffixed = 255;
    } else if(strstr(buffer, "GET /stream.html?/tilt_cw") != NULL) {
        req.type = A_TILT_CW;
        input_suffixed = 255;
    } else if(strstr(buffer, "GET /stream.html?/pan_cw") != NULL) {
        req.type = A_PAN_CW;
        input_suffixed = 255;
    } else if(strstr(buffer, "GET /stream.html?/pan_ccw") != NULL) {
        req.type = A_PAN_CCW;
        input_suffixed = 255;
    }
